...
def test_main_writes_conda_metadata_and_summary_lines(monkeypatch, tmp_path):
    ...
    report["foldseek"] = {
        "bin_path": {"status": "manual", "path": "__MANUAL__: set Foldseek binary path"},
        "database_path": {"status": "manual", "path": "__MANUAL__: set Foldseek database path"},
    }
...
def test_add_candidate_sections_records_protrek_runtime_and_foldseek(monkeypatch, tmp_path):
    ...
    foldseek_bin = tmp_path / "mnt" / "disk3" / "foldseek" / "bin" / "foldseek"
    foldseek_db = tmp_path / "mnt" / "disk3" / "foldseek" / "db" / "afdb50"

    protrek_python.parent.mkdir(parents=True)
    weights_dir.mkdir(parents=True)
    data_root.mkdir(parents=True)
    runs_root.mkdir(parents=True)
    foldseek_bin.parent.mkdir(parents=True)
    foldseek_db.mkdir(parents=True)
    protrek_python.write_text("", encoding="utf-8")
    foldseek_bin.write_text("", encoding="utf-8")
    ...
    monkeypatch.setattr(probe, "FOLDSEEK_BIN_CANDIDATES", [foldseek_bin])
    monkeypatch.setattr(probe, "FOLDSEEK_DATABASE_CANDIDATES", [foldseek_db])
    ...
    assert report["foldseek"]["bin_path"]["status"] == "candidate"
    assert report["foldseek"]["bin_path"]["path"] == str(foldseek_bin)
    assert report["foldseek"]["database_path"]["status"] == "candidate"
    assert report["foldseek"]["database_path"]["path"] == str(foldseek_db)
    assert report["warnings"] == []
...
def test_main_writes_json_yaml_and_summary_bundle(monkeypatch, tmp_path):
    ...
    assert 'foldseek_bin: "__MANUAL__: set Foldseek binary path"' in yaml_text
    assert 'foldseek_database_path: "__MANUAL__: set Foldseek database path"' in yaml_text

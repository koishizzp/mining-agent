...
def run_seeded_pipeline(
    ...
    seed_recall_hash = _combined_resume_hash(seed_faa, cluster_rep_faa)

    sequence_done = stage_dirs["seed_sequence_recall"] / "DONE.json"
    if should_skip_stage(sequence_done, seed_recall_hash, resume):
    ...
    if should_skip_stage(structure_done, seed_recall_hash, resume):
    ...
    merge_hash = _combined_resume_hash(
        cluster_rep_faa,
        sequence_result["sequence_hits_tsv"],
        structure_result["structure_hits_tsv"],
    )
    if (
        should_skip_stage(merge_done, merge_hash, resume)
        and seed_manifest_tsv.exists()
        and seeded_targets_faa.exists()
    ):
        merge_result = {
    ...
    thermo_input = Path(merge_result["seeded_targets_faa"])
    thermo_done = stage_dirs["temstapro_screen"] / "DONE.json"
    if should_skip_stage(thermo_done, sha256_file(thermo_input), resume):
        thermo_result = {
            "thermo_hits_faa": stage_dirs["temstapro_screen"] / "thermo_hits.faa",
    ...
    structure_input = Path(thermo_result["thermo_hits_faa"])
    structure_done = stage_dirs["structure_predict"] / "DONE.json"
    structure_manifest_path = stage_dirs["structure_predict"] / "structure_manifest.json"
    if should_skip_stage(structure_done, sha256_file(structure_input), resume) and structure_manifest_path.exists():
        final_structure_result = {
            "structure_manifest": json.loads(structure_manifest_path.read_text(encoding="utf-8")),
            "structure_manifest_json": structure_manifest_path,
        }
    else:
        final_structure_result = run_structure_predict_stage(
            input_faa=structure_input,
            stage_dir=stage_dirs["structure_predict"],
            colabfold_batch_bin=settings.tools.colabfold_batch_bin,
            colabfold_data_dir=settings.tools.colabfold_data_dir,
            msa_mode=settings.defaults.colabfold_msa_mode,
